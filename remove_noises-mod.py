...
def load_mat(path, fill = 0.0):
  ...
  feature = np.append(feature, np.array([fill], dtype=np.float32))
  feature = preproc.normalize(feature.reshape(-1, 1))
...
def main(args):
  ...
  for line in open(args.facescrub_lst, 'r'):
    ...
    feature_path = Path(args.feature_dir_input) / 'facescrub' / a / PurePath(b).with_suffix('.mat')
  ...
  for line in open(args.megaface_lst, 'r'):
    ...
    # feature_path = os.path.join(args.feature_dir_input, 'megaface', a1, a2, a, "_%s.bin"%(b))
    feature_path = Path(args.feature_dir_input) / 'megaface' / a1 / a2 / PurePath(b).with_suffix('.mat')
    ...
    if not bb in mf_noise_map:
      ...
      feature = load_mat(feature_path, 100.0)
      write_bin(feature_path_out, feature)
